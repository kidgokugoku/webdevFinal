...
const processCourses = (data) => {
  let newArray = data.reduce((total, cur, index) => {
    ...
    if (hasValue === -1) {
      ...
      total.push(cur)
    } else if (!total[hasValue].teacherName.includes(cur.teacherName))
...
const CourseSelectionTeacher = () => {
  const { userStore } = useStore()
  const [data, setData] = useState()
  const [loading, setLoading] = useState(false)
  const [dataUpdateCounter, setDataUpdateCounter] = useState(0)

  const confirmSelection = async (courseId, teacherId) => {
    let data = new FormData()
    data.append('courseId', courseId)
    data.append('teacherId', teacherId)
    const res = await http.post(
      'http://localhost:8080/api/v1/courses/TeacherSelect',
      data
    )
    if (res.data.state === 200) {
  ...
  const confirmDeselection = async (courseId, teacherId) => {
  ...
  const columns = [
...

    {
      ...
      // @ts-ignore
      render: (_, record) => {
        return record.teacherName.includes(userStore.userInfo.name) ? (
          <span>
            <Popconfirm
          ...
          <span>
            <Popconfirm
              ...
              cancelText="No"
              onConfirm={() => {
                confirmSelection(record.key, userStore.userInfo.uid)
              }}
            >
...
export default CourseSelectionTeacher
